function neuronPairsRefVsNew = crossIdentifyNeuronIDs_NB(dataFolderRef, dataFolderNew, refNeuronIDs, varargin)
% crossIdentifyNeuronIDs(dataFolderRef,dataFolderNew,...
%                        refNeuronIDs, optional)
%
...
%
% Optional:
%    - nSim: number of matches from the new dataset to return (default 1)
%    - likelihood threshold, can change if you aren't getting enough
%    matches (default 45000... legacy number)
%    - autoClassify ? (default 0)
%    - newNeuronIDs: if this lists is specified, only
%    the neurons in this list will be considered for pairing, default all
%    neurons
%
% Returns:
...
%
% Version: 2016-07-25 NB

%% Parameters
p = inputParser;
p.addParameter('newNeuronIDs', [])
p.addParameter('autoClassify', 0)
p.addParameter('nSim', 1); % If you want to output more than one match candidate for each neuron, you can increase nSim
p.addParameter('threshold', 45000)
p.parse(varargin{:});
newNeuronIDs = p.Results.newNeuronIDs;
autoClassify = p.Results.autoClassify;
nSim = p.Results.nSim;
...
if isempty(contentsDataFolder); contentsDataFolder = dir(dataFolderNew); end
for kk=1:length(contentsDataFolder)
    isNeuronFile = strfind(contentsDataFolder(kk).name,'.neurons');
    isNeuronRawFile = strfind(contentsDataFolder(kk).name,'.neurons-raw');
    isEIFile = strfind(contentsDataFolder(kk).name,'.ei');
