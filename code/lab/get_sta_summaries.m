...
% sta                   []          empty if sta is stored in datarun or 4D
%                                   single if STA is loaded from matlab
%                                   (x,y,colors, time)
% 
%
...
p.addParamValue('keep_rf_coms', false);
p.addParamValue('sta', []);
...
for cc=1:length(cell_indices)
    ...
    % get the STA
    %if ~any(strcmp('sta',  fieldnames(params)))
...
%     end
    ...
    % remove boader
    full_sig = full(sig_stixels);
    remove_border = full_sig(10:end-9, 10:end-9);
    resize_sig = zeros(size(sta,1), size(sta,2));
    resize_sig(10:end-9, 10:end-9) = remove_border;
    sig_stixels = sparse(logical(resize_sig));
    
    % if they're empty, try some spatial filtering!
    if sum(sum(sig_stixels)) == 0
        % generate small filter
        filt = make_gaussian('dim',2,'center_radius',1,'x_size',10,'y_size',10);
        % apply it
