...
parfor rgc = 1:num_rgcs
    ...
    fit_array{rgc} = temp_fit_params;
    
end

% if this field is not present
if ~isfield(datarun, 'matlab')
    datarun.matlab.sta.fits = cell(1,length(datarun.cell_ids));
end

for rgc = 1:num_rgcs
    datarun.matlab.sta_fits{cell_indices(rgc)} = fit_array{rgc};
end
