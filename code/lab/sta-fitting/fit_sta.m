...
%
%   fit_sig_stixels_only             false   only fit significant stixels
%
...
p.addParamValue('initial_n_one_filters', 9, @isnumeric);
p.addParamValue('initial_n_two_filters', 9, @isnumeric);
...
% fiting options
p.addParamValue('optim',{'TolFun',0.00001,'Display','off', 'MaxIter', 100000, 'MaxFunEvals', 100000});

% Sig stixels option
p.addParamValue('biggest_blob', false, @islogical);
...
if size(matrix_subscripts_eig,1) >= 3
    ...
    [correlation_matrix, ~, eigvalues] = pca(matrix_subscripts_eig);
