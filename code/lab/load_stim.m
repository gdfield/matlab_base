...
p = inputParser;
p.addParameter('ignor_field', []);%eg {'SEED'}
p.addParameter('change_entry', []);% struct('field','SEED','find',{11112,11113},'replace',{11111,11111})
p.addParameter('find_trigger', 1);
p.addParameter('trigger_iti_thr', 2);
p.addParameter('plot', 0);
p.addParameter('correction_incomplet_run', 1);
p.addParameter('user_defined_trigger_interval', [], @isnumeric); % Caution, this might not work if frames were dropped
p.addParameter('user_defined_trigger_set', [], @isnumeric);
...
for i=j:length(l)
        if ~isempty(strfind(l{i},':'))
            ...
            if ~isempty(str2double((l{i+1}))) | isequal(l{i+1},'0')
                s.trials(jj).(t)=str2double(l{i+1});
            else
                s.trials(jj).(t)=(l{i+1});
            end
            n=2;
            while i+n<=length(l) & isempty(strfind(l{i+n},':'))
                if ~isempty(str2double((l{i+n})))
                    s.trials(jj).(t)=[s.trials(jj).(t) str2double(l{i+n})];
                else
