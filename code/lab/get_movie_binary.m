function [mov,height,width,duration,refresh] = get_movie_binary(mdf_file, triggers, frames)
...
if length(tduration) > 1
    mov=zeros(height,width,3,length(tduration), 'int8');
    cntr = 0;
    for i=tduration(1):tduration(length(tduration))
        ...
        cntr = cntr + 1;
        mov(:,:,:,cntr) = int8(round(permute(F,[3 2 1])));
    end
else
    mov=zeros(height,width,3,tduration, 'int8');
    for i=1:tduration
        % grab movie frame
        F = mvi.getFrame(i-1).getBuffer;
        ...
        F = reshape(F,3,width,height);
        mov(:,:,:,i) = int8(round(permute(F,[3 2 1])));
    end
end
