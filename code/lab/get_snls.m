...
% matlab_movie      []              use a matlab movie instead of a java movie if provided 
% interval          []              must be used if giving a matlab movie
% x_dim             []              must be used if giving a matlab movie
% y_dim             []              must be used if giving a matlab movie
%
...
p.addParameter('movie', []);
p.addParameter('frames', ':');
p.addParameter('new', false);
p.addParameter('stimuli', []);
p.addParameter('start_time', 0, @(x)numel(x)==1 && x>=0 && x <= datarun.duration);
p.addParameter('end_time', datarun.duration, @(x)numel(x) == 1);
p.addParameter('marks', 'marks');
p.addParameter('simplemarkthresh', 4);
p.addParameter('matlab_movie', []);
p.addParameter('interval', []);
p.addParameter('x_dim', []);
p.addParameter('y_dim', []);
...
if ~isempty(params.matlab_movie)
    movie = params.matlab_movie;

elseif isempty(params.movie)
...
if ~isempty(params.matlab_movie)
    ...
    refresh_time = 100/119.5*param.interval/1000;
    
    % note stimulus size
    field_width = param.x_dim;
    field_height =param.y_dim;
    size_movie = size(movie,4);
    spikes = sparse(zeros(size(movie,4),length(cell_indices)));
...
%%
    ...
    % compute spike rate at all times
    spike_rate = histc(datarun.spikes{cell_index},datarun.triggers(1):refresh_time:size_movie*refresh_time+datarun.triggers(1));
    % store spikes in the relevant region
...
start_stim = floor(1+params.start_time/(refresh_time));
end_stim = floor(1+params.end_time/(refresh_time));
...
% GDF edition on 6/24/2017
for rgc = 1:length(strfs)
    tmp_strf = strfs{rgc};
    tmp_strf = tmp_strf ./ norm(tmp_strf(:));
    strfs{rgc} = tmp_strf;
end
...
% compute SNLs
[fit_params, snl_params] = compute_snls(spikes, gen_signals, 'fit', p.Results.fit);
