...
p.addParamValue('PSTH_smoothing', 120)
p.parse(varargin{:});
...
xvalperformance.rasters.bintime  = params.bindur;
    convolve = gausswin(smoothing)/(params.trials*fittedGLM.t_bin*sum(gausswin(smoothing)));
    LN_FR = conv(sum(fittedGLM.xvalperformance.rasters.glm_sim),convolve,'same');
    GLM_FR = conv(sum(fittedGLM.xvalperformance.rasters.glm_sim),convolve,'same');
    REC_FR = conv(sum(fittedGLM.xvalperformance.rasters.recorded),convolve,'same');
    firing_even = conv(sum(fittedGLM.xvalperformance.rasters.recorded(2:2:end,:)),convolve,'same');
    firing_odd = conv(sum(fittedGLM.xvalperformance.rasters.recorded(1:2:end,:)),convolve,'same');
    ...
    fittedGLM.xvalperformance.orig_glm.FEV = (1-A_GLM/B)/(1-C/D);
    fittedGLM.xvalperformance.FEV = (1-A_LN/B)/(1-C/D);
end
