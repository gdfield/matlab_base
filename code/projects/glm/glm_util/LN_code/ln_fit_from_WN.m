...
function [fittedLN] = ln_fit_from_WN(cells, dataset, stim, varargin)
% DESCRIPTION
...
fitmovie = fitmovie(:,:,start_frame:end);
...
for i_cell = 1:length(cells)
    ...
    center = vision_fit_to_movie_center(datarun.vision.sta_fits{master_idx}.mean, [width height], [width height]);
    %center = round(datarun.vision.sta_fits{master_idx}.mean)
...
%   
    ...
    start_time = (start_frame-1)/monitor_refresh;
    fitspikes = fitspikes(fitspikes>start_time)-start_time;

    [STA, ~] = STA_Test(fitspikes, fitmovie, 0, 1/monitor_refresh);
    
    % Execute and save GLM
    tic
    fittedLN     = ln_fit(fitspikes, fitmovie, center, 'WN_STA', STA, 'monitor_refresh', monitor_refresh);
    toc
    
    fittedLN.STA = STA;
    fittedLN.center = center;
    
    if GLMT.CouplingFilters
        fittedLN.paired_cells = paired_cells;
    end
    
    if testmovie
        if isstr(d_save)
            fittedLN = save_ln(fittedLN, p.Results.testspikes(:,i_cell), p.Results.testmovie, [d_save '/' glm_cellinfo.cell_savename]);
        else
