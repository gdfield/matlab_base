function [model, t_init, increments] = fit_NL_and_K(fittedGLM, fitspikes, fitmovie, varargin)
% This takes in a fittedGLM structure and makes it better!!!!
% NJB 2016-06-03
% JK but it does iteratively fit a sigmoidal nonlinearity and a timecourse and a spatial filter.
% Boom. 
...
space_init = [ceil(roi_length/2), ceil(roi_length/2), 3, 6, 0];
increments.NL.start = NL_init;
increments.time.start = t_init;
increments.space.start = space_init;
...
for iter = 1:iterations
    ...
    new_time = fminunc(@(combo_filter)K_error_function(combo_filter, firing_rate, model, fittedGLM, fitmovie, 0), double([t_init; space_init(:)]));
    t_init = new_time(1:30); % set the initial values for the next iteration
    space_init = new_time(30:end);
    space = space_init;
    clear new_time;
    ...
    fittedGLM.linearfilters.Stimulus.space_rk1 = space;
    ...
    increments.space.iter{iter} = space_init;
    toc
end
end