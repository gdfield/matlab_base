...
function [fittedGLM] = ln_fit_from_WN_exp(cells, dataset, stim, varargin)
% DESCRIPTION
...
for i_cell = 1:length(cells)
    ...
    center = vision_fit_to_movie_center(datarun.vision.sta_fits{master_idx}.mean, [80 40], [80 40]);
...
%   
    ...
    tic
    fittedGLM     = ln_fit(fitspikes, fitmovie, center, 'WN_STA', STA, 'monitor_refresh', monitor_refresh, 'exp', 1);
    toc
    ...
    if testmovie
         fittedGLM = ln_predict(fittedGLM, p.Results.testmovie,'testspikes', p.Results.testspikes(:,i_cell));
