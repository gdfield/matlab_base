...
function [fittedGLM] = ln_fit_from_WN_GStest(cells, dataset, stim, cell_type_short, varargin)
% DESCRIPTION
...
if p.Results.neighborspikes ~= 0
    warning('LN fit does not include coupling. Neighbor spikes will be ignored')
end
...
monitor_refresh = frames_per_trigger/median(diff(datarun.triggers));
...
for i_cell = 1:length(cells)
    ...
    fitspikes = align_spikes_triggers(datarun.spikes{master_idx}, datarun.triggers, 100);
    ...
    dsave_NSEM = ['/Volumes/Lab/Users/Nora/LNFits/RPE/201602171/NSEM/' cell_type_short];
    eval(sprintf('load %s/%s.mat', dsave_NSEM, glm_cellinfo.cell_savename));
    model = fittedLN.model;
    center = [fittedLN.orig_glm.center_coord.x_coord fittedLN.orig_glm.center_coord.y_coord];
    clear fittedLN
    ...
    disp('fitting NL and time iteratively')
    [time, iterations] = fit_time(fittedGLM, fitspikes, fitmovie,model, 'iterations', 1);
    
    % structure that shit
    fittedLN.orig_glm = fittedGLM;
    fittedLN.model = model;
    fittedLN.new_NL.firing_rate = predict(model, [-2:0.5:2]');
    fittedLN.new_NL.gen_signal = [-2:0.5:2]';
    fittedLN.new_time = time;
    fittedLN.iterations = iterations;
    toc
    
    
    if testmovie
        fittedLN = ln_predict(fittedLN, p.Results.testmovie,'testspikes', p.Results.testspikes(:,i_cell));
    end
    
    if isstr(d_save)
        mkdir(d_save)
        eval(sprintf('save %s/%s.mat fittedLN', d_save, glm_cellinfo.cell_savename));
        close all
        %%{
