function [model, t_init, increments] = fit_NL_and_time(fittedGLM, fitspikes, fitmovie, varargin)
...
p.addParameter('n_bins_generator_signal', 50);
p.addParameter('use_exp', 0);
p.parse(varargin{:});
NL_init = p.Results.NL_init;
iterations = p.Results.iterations;
n_bins = p.Results.n_bins_generator_signal;
use_exp = p.Results.use_exp;
clear p
tic
...
for iter = 1:iterations
    ...
    if use_exp
        ...
        model = fitnlm(gen_signal, firing_rate, 'y~b4+b1/(b2+exp(b3*x1))', NL_init); % Use matlab's fun model fitting to fit the sigmoidal NL
    end
