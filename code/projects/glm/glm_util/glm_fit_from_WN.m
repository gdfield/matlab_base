...
%     center(2) = size(fitmovie,2) - round(center_coord(2)); %y_coord
    % convert vision center location to coordates used by STA_Test
    center_coord = datarun.vision.sta_fits{master_idx}.mean;
    center(2) = round(center_coord(1)); % x_coord
    center(1) = size(fitmovie,1) - round(center_coord(2)); %y_coord
%     clear cell_savename
%   
    ...
    % Execute and save GLM
%     tic
%     % Use center from vision STA
%     fittedGLM     = glm_fit(fitspikes, fitmovie, center, 'monitor_refresh', monitor_refresh, 'neighborspikes', neighborspikes);
%     toc
...
=======
         fittedGLM.xvalperformance = glm_predict(fittedGLM, p.Results.testmovie,'testspikes', p.Results.testspikes(:,i_cell), 'neighborspikes',p.Results.neighborspikes, 'trials', 30);
         temp = corrcoef(conv(sum(fittedGLM.xvalperformance.rasters.glm_sim), gausswin(100)),conv(sum(fittedGLM.xvalperformance.rasters.recorded), gausswin(100)));
         fittedGLM.xvalperformance.corr = temp(2,1);
         
         
>>>>>>> 5a475bf9887df22db3eea1cca7bec9d8b2e8919e
    end
...
<<<<<<< HEAD
        ...
        %}
=======
            plotrasters(fittedGLM.xvalperformance, fittedGLM)%, 'start_time',0, 'raster_length',10);
            title({['Cell: ', num2str(cid)];['Correlation coefficent: ',num2str(fittedGLM.xvalperformance.corr)]})
            exportfig(gcf, [d_save '/' glm_cellinfo.cell_savename '_rasters'], 'Bounds', 'loose', 'Color', 'rgb');
        end
