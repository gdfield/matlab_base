...
if isstr(cells)
   cells = get_cell_ids(datarun, cells); 
end
datarun = load_sta(datarun, 'load_sta', cells);
...
%   
    ...
    fitspikes = align_spikes_triggers(datarun.spikes{master_idx}, datarun.triggers, frames_per_trigger/interval);
    ...
    close all
    %[STA, center] = STA_Test(fitspikes, fitmovie, center_verification, interval/monitor_refresh);
    
    % Execute and save GLM
    tic
    fittedGLM     = glm_fit(fitspikes, fitmovie, center, 'WN_STA', STA, 'monitor_refresh', monitor_refresh/interval, 'neighborspikes', neighborspikes, 'GLM_params', p.Results.GLM_params);
    toc
    ...
    if isstr(d_save)
        ...
        exportfig(gcf, [d_save '/' glm_cellinfo.cell_savename '_filters'], 'Bounds', 'loose', 'Color', 'rgb');
        if testmovie
