...
% NB 2015-06-18
% DESCRIPTION
% This function takes in xval from glm_predict or ln_predict, fittedGLM from glm_fit, and
...
addParameter(p,'raster_length',10, @isnumeric); % length of raster in seconds, default 5
...
addParameter(p, 'plot', 1, @islogical);
...
dt = fittedGLM.t_bin;
bins     = round(1/dt * p.Results.raster_length) ;
start_bin = round(1/dt * p.Results.start_time  + 1);
...
try
    ...
    try
        sim_rast = xval.rasters.ln_sim(:,bins_idx);
    catch
        sim_rast = xval.rasters.glm_sim(:,bins_idx);
    end
catch
    bins_idx= 1:length(xval.rasters.recorded);
    time     = dt*(bins_idx);
    if ~predict_only
        rec_rast = xval.rasters.recorded(:,bins_idx);
    end
