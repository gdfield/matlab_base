function wholeArrayAutoAnalysis_biElec(eiFilePath, pathToAnalysisData,varargin)
...
startPattern = 1; 
d = dir(fullfile(pathToAnalysisData, 'p*')); % Look at the folders starting with p, should equal the pattern numbers
endPattern = size(d,1)-1; 
threshold = 10; 
...
patternNo = startPattern; 
try
    elecsUsed = getElecsFromPatternFiles([fullfile(pathToAnalysisData,'pattern_files') filesep]);
    for e = startPattern:endPattern
        % Get stimulating electrodes associated with the specified pattern
        electrodes = full(elecsUsed(e,:)); 
        cellIDs = [];
        for ee = 1:length(electrodes)
            cellIDs = [cellIDs; cellsToAnalyze{electrodes(ee)}] ;
        end
    ...
    disp(['Final pattern analyzed was ' num2str(patternNo)'])
    rethrow(ME)
end