...
%   patternNumber: 
%   movieNumber: 
% optional: numElectrodes 512,61, or 519: This variable is only used to
%                       generate global constants, which gives the current ranges for the chip.
%                       since these are the same on all of our systems, this optional variable
%                       isn't really required, but is left here in case
%                       future requirements make it useful. 
%                       
...
for ii = 1:length(thisPatternLong)
    if any(thisPatternLong(ii).data(3,:) ~= 0); %if "connect" signal is nonzero at some point
        channelsConnected = [channelsConnected thisPatternLong(ii).channel]; %#ok<AGROW>
    end
end
