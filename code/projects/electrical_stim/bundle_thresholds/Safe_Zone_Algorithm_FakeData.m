...
DataName = '2015-11-09-3';
% DataName = '2015-10-06-3';

switch DataName
    case '2015-11-09-3'
        if isdir('/Volumes/Analysis/') && isdir('/Volumes/Lab/')
            dataset = '/Volumes/Analysis/2015-11-09-3/data001-data002/';
            fakePath = '/Volumes/Lab/Projects/electrical_stim/bundle-safe-zones/synthetic_data/2015-11-09-3-data001-data002-oneAmpBelow/';
        elseif isdir('/Volumes/Seagate Backup Plus Drive/')
            dataset = '/Volumes/Seagate Backup Plus Drive/datasets/2015-11-09-3/data001-data002/';
            fakePath = '/Volumes/Seagate Backup Plus Drive/bundle-paper/synthetic-data/2015-11-09-3-data001-data002-oneAmpBelow/';
        else
            disp('Must connect to correct data drive');
        end
        DominantBand = 1; % Human knows dominant band (direction against axons)
        AxonDirection = 2; % Only for computing max-hops: 1 up-down, 2 left-right
        
    case '2015-10-06-3'
        if isdir('/Volumes/Analysis/') && isdir('/Volumes/Lab/')
            dataset = '/Volumes/Analysis/2015-10-06-3/data001-data002/';
            fakePath = '/Volumes/Lab/Projects/electrical_stim/bundle-safe-zones/synthetic_data/2015-10-06-3-data001-data002-oneAmpBelow-50p/';
            
        elseif isdir('/Volumes/Seagate Backup Plus Drive/')
            dataset = '/Volumes/Seagate Backup Plus Drive/datasets/2015-10-06-3/data001-data002/';
            fakePath= '/Volumes/Seagate Backup Plus Drive/bundle-paper/synthetic-data/2015-10-06-3-data001-data002-oneAmpBelow/';
        else
            disp('Must connect to correct data drive');
        end
        DominantBand = 1; % Human knows dominant band (direction against axons)
        AxonDirection = 1; % Only for computing max-hops: 1 up-down, 2 left-right
end
...
%% Step 7 read in the files
dirinfo = dir(fakePath);

% First two files are . and ..
Ground_Output = zeros(length(dirinfo)-2,1);
Algorithm_Output = zeros(length(dirinfo)-2,1);
...
alleisizes = zeros(size(dirinfo,1)-2,1);

for i = 3:length(dirinfo) % now for each of these files we need to run the algorithm
    load([fakePath dirinfo(i).name]);
    modData = modifiedData.data;
    stimElec = modifiedData.stimElec;
    firstMovie = modifiedData.firstMovieWithAddedEi;
    percentTrials = modifiedData.percentageOfTrials;
    distanceToCell = modifiedData.distance_stimElec_to_soma;
    
    Ground_Output(i-2) = firstMovie; % Save the ground truth now
    
    % Extract data parameters from the mat file
    [NumRepeats, NumElectrodes, NumSamples, stimAmpSize] = size(modData);
    
    % Step 8 : Extract Bands, centres of mass, pairwise velocities (magnitude and angles),
    % and the closest electrodes to each center of mass
    Bands = cell(1,2);
    
    Velocities1 = zeros(stimAmpSize,NumSamples-1,2);
    Velocities2 = zeros(stimAmpSize,NumSamples-1,2);
    
    Centers1 = zeros(stimAmpSize,NumSamples,2);
    Centers2 = zeros(stimAmpSize,NumSamples,2);
    
    closestElecInfo1 = cell(stimAmpSize);
    closestElecInfo2 = cell(stimAmpSize);
    
    % Create and store (as temp) nonlinear filters to apply to the data
    NonlinearFilter5 = cell(stimAmpSize);
    ...
    firstArtifact = mean(modData(:,:,:,1),1);
    subtractionMatrix = repmat(firstArtifact,[NumRepeats 1]);
                
    for stimAmp = 2:1:stimAmpSize
        % Now create the nonlinear filter...
        meanData = squeeze(mean(modData(:, :, :, stimAmp) - subtractionMatrix, 1)); % Full data after artifact subtraction
        meanDataClean = meanData(:,1:25); % Dont count stuff after sample 25.
        [B,I] = sort(meanDataClean');
        orderedData = B';
        orderingData = I';
        orderingData = orderingData(:,1:5);
        % Create the Nonlinear Filter
        NonlinearFilter5{stimAmp} = orderingData;
        
        for t = 1:NumSamples
            if t > 1 && (Centers1(stimAmp,t-1,1)~=xStim || Centers1(stimAmp,t-1,2)~=yStim)
            ...
            if t > 1 && (Centers2(stimAmp,t-1,1)~=xStim || Centers2(stimAmp,t-1,2)~=yStim)
            ...
            if ~isempty(Band1_Current)
                meanData_5 = meanData(:,t).*sum(ismember(NonlinearFilter5{stimAmp}, t), 2);
                ...
                xDistance1 = reshape(xDistance1, size(data1));
                yDistance1 = reshape(yDistance1, size(data1));
                Centers1(stimAmp,t,1) = sum(data1.*xDistance1)./sum(data1) + xStim;
                Centers1(stimAmp,t,2) = sum(data1.*yDistance1)./sum(data1) + yStim;
                
                if (isnan(Centers1(stimAmp,t,1)) || isnan(Centers1(stimAmp,t,2)))
                    Centers1(stimAmp,t,1) = xStim;
                    Centers1(stimAmp,t,2) = yStim;
                end
                
                diffs = sqrt((Centers1(stimAmp,t,1)-xCoords).^2 + (Centers1(stimAmp,t,2)-yCoords).^2);
                [M,I] = min(diffs);
                Closest1 = I;
                centerQ = hexCoords(Closest1, 1);
                centerR = hexCoords(Closest1, 2);
                
                closestElecInfo1{stimAmp} = [closestElecInfo1{stimAmp}, elecHexDist(startQ, startR, centerQ, centerR)];
            else
                Centers1(stimAmp,t,1) = xStim;
                Centers1(stimAmp,t,2) = yStim;
                closestElecInfo1{stimAmp} = [closestElecInfo1{stimAmp}, 0];
                Closest1 = stimElec;
            end
            
            if ~isempty(Band2_Current)
%                 firstArtifact = mean(squeeze(modData(:,Band2_Current,t,1)),1);
%                 subtractionMatrix = repmat(firstArtifact, [NumRepeats 1]);
                meanData_5 = meanData(:,t).*sum(ismember(NonlinearFilter5{stimAmp}, t), 2);
                data22 = meanData_5(Band2_Current); % Take only current band elecs
%                 data22 = squeeze(mean(modData(:, Band2_Current, t, stimAmp) - subtractionMatrix, 1));
                data2 = -1*data22.*[data22<0];
                
                xDistance2 = xCoords(Band2_Current) - xStim;
                yDistance2 = yCoords(Band2_Current) - yStim;
                
                xDistance2 = reshape(xDistance2, size(data2));
                yDistance2 = reshape(yDistance2, size(data2));
                Centers2(stimAmp,t,1) = sum(data2.*xDistance2)./sum(data2) + xStim;
                Centers2(stimAmp,t,2) = sum(data2.*yDistance2)./sum(data2) + yStim;
                
                if (isnan(Centers2(stimAmp,t,1)) || isnan(Centers2(stimAmp,t,2)))
                    Centers2(stimAmp,t,1) = xStim;
                    Centers2(stimAmp,t,2) = yStim;
                end
                
                diffs = sqrt((Centers2(stimAmp,t,1)-xCoords).^2 + (Centers2(stimAmp,t,2)-yCoords).^2);
                [M,I] = min(diffs);
                Closest2 = I;
                centerQ = hexCoords(Closest2, 1);
                centerR = hexCoords(Closest2, 2);
                
                closestElecInfo2{stimAmp} = [closestElecInfo2{stimAmp},elecHexDist(startQ, startR, centerQ, centerR)];
            else
                Centers2(stimAmp,t,1) = xStim;
                Centers2(stimAmp,t,2) = yStim;
                closestElecInfo2{stimAmp} = [closestElecInfo2{stimAmp}, 0];
                Closest2 = stimElec;
            end
        end
        for t = 1:NumSamples-1
            if ~isempty(Band1_Current)
                Velocities1(stimAmp,t,1) = norm(squeeze(Centers1(stimAmp,t+1,:) - Centers1(stimAmp,t,:)));
                ang = rad2deg(atan2(squeeze(Centers1(stimAmp,t+1,2)-Centers1(stimAmp,t,2)),...
                    squeeze(Centers1(stimAmp,t+1,1)-Centers1(stimAmp,t,1))));
                
                % convert angles to be in range [0,360]
                Velocities1(stimAmp,t,2) = ang.*[ang >= 0] + [ang < 0].*[ang + 360];
            end
            if ~isempty(Band2_Current)
                Velocities2(stimAmp,t,1) = norm(squeeze(Centers2(stimAmp,t+1,:) - Centers2(stimAmp,t,:)));
                ang = rad2deg(atan2(squeeze(Centers2(stimAmp,t+1,2)-Centers2(stimAmp,t,2)),...
                    squeeze(Centers2(stimAmp,t+1,1)-Centers2(stimAmp,t,1))));
                
                % convert angles to be in range [0,360]
                Velocities2(stimAmp,t,2) = ang.*[ang >= 0] + [ang < 0].*[ang + 360];
            end
        end
    end
    disp(['Bands, Centers, Velocities for pattern ' num2str(stimElec)]);
    
    % Step 9: Compute angles relative to the band
    ...
    for j = 1:B
        for t = 1:C
            relAngles1(j,t) = mod(Velocities1(j,t,2) - perpangles(partition(stimElec)),360);
            relAngles2(j,t) = mod(Velocities2(j,t,2) - perpangles(partition(stimElec)),360);
        end
    end
    disp(['Relative Angles for pattern ' num2str(stimElec)]);
    
    % Step 10: Extract displacements
    displacement1 = zeros(stimAmpSize,NumSamples);
    displacement2 = zeros(stimAmpSize,NumSamples);
    
    for j = 1:stimAmpSize
        for t = 1:NumSamples
            displacement1(j,t) = norm([(Centers1(j,t,1)-xStim) (Centers1(j,t,2)-yStim)]);
            displacement2(j,t) = norm([(Centers2(j,t,1)-xStim) (Centers2(j,t,2)-yStim)]);
        end
    end
    disp(['Displacements for pattern ' num2str(stimElec)]);
    
    % Step 11: Time Window Computation
    % uses function monotoneByDiff
    % uses function angleSign
    
    timeWindow1MA = cell(stimAmpSize);
    timeWindow2MA = cell(stimAmpSize);
    ...
    for j = stimAmpSize:-1:2
        % Band 1
        mInc1 = monotoneByDiff(squeeze(closestElecInfo1{j})); % check monotonicity
        aInt1 = angleInterval(squeeze(relAngles1(j,:)));
        aInt1(aInt1 == -1) = 0; % If this angle is between 180, not
        aInt1 = aInt1(1:25);
        mInc1 = mInc1(1:25);
        aInt1 = reshape(aInt1, size(mInc1));
        ...
        startHops1 = squeeze(closestElecInfo1{j}(check1Starts));
        endHops1 = squeeze(closestElecInfo1{j}(check1Starts+check1Runs-1));
        ...
        if (movAvEnd1==0) && (movAvStart1==0)
            [M,I] = max(check1Runs);
            if M~=0
                timeWindow1MA{j} = check1Starts(I):1:(check1Starts(I)+check1Runs(I)-1);
                movAvStart1 = timeWindow1MA{j}(1);
                movAvEnd1 = timeWindow1MA{j}(end);
            ...
            for q = 1:1:length(check1Starts)
                ...
                relevanthops1 = squeeze(closestElecInfo1{j}(timestoCheck1));
            ...
            if M~=0 % If there is something with a non-trivial intersection, use it for now
                ...
                if (stableTW1ctr < 3) % use only the last 3 "good" amplitudes for estimating TW
                    movAvStartArray1 = [timeWindow1MA{j}(1) movAvStartArray1];
                    movAvEndArray1 = [timeWindow1MA{j}(end) movAvEndArray1];
        ...
        % Band 2
        mInc2 = monotoneByDiff(squeeze(closestElecInfo2{j})); % check monotonicity
        aInt2 = angleInterval(squeeze(relAngles2(j,:)));
        aInt2(aInt2 == 1) = 0;
        aInt2 = aInt2(1:25);
        mInc2 = mInc2(1:25);
        
        aInt2 = reshape(aInt2, size(mInc2));
        ...
        startHops2 = squeeze(closestElecInfo2{j}(check2Starts));
        endHops2 = squeeze(closestElecInfo2{j}(check2Starts+check2Runs-1));
        ...
        if (movAvEnd2==0) && (movAvStart2==0)
            [M,I] = max(check2Runs);
            if M~=0
                timeWindow2MA{j} = check2Starts(I):1:(check2Starts(I)+check2Runs(I)-1);
                movAvStart2 = timeWindow2MA{j}(1);
                movAvEnd2 = timeWindow2MA{j}(end);
            ...
            for q = 1:1:length(check2Starts)
                ...
                relevanthops2 = squeeze(closestElecInfo2{j}(timestoCheck2));
            ...
            if M~=0 % If there is something with a non-trivial intersection, use it for now
                ...
                if (stableTW2ctr < 3) % use only the last 3 "good" amplitudes for estimating TW
                    movAvStartArray2 = [timeWindow2MA{j}(1) movAvStartArray2];
                    movAvEndArray2 = [timeWindow2MA{j}(end) movAvEndArray2];
    ...
    baseWindow1 = round(movAvStart1):1:round(movAvEnd1);
    baseWindow2 = round(movAvStart2):1:round(movAvEnd2);
    disp(['Time Windows for pattern ' num2str(stimElec)]);
    
    % Step 12: Feature Extraction
    startTimes = zeros(stimAmpSize,2);
    endTimes = zeros(stimAmpSize,2);
    totalJump = zeros(stimAmpSize,2);
    startHop = zeros(stimAmpSize,2);
    
    for j = stimAmpSize:-1:2
        if ~isempty(timeWindow1MA{j})
            startTimes(j,1) = timeWindow1MA{j}(1);
            endTimes(j,1) = timeWindow1MA{j}(end);
            totalJump(j,1) = closestElecInfo1{j}(timeWindow1MA{j}(end)) - closestElecInfo1{j}(timeWindow1MA{j}(1));
            startHop(j,1) = closestElecInfo1{j}(timeWindow1MA{j}(1));
        end
        if ~isempty(timeWindow2MA{j})
            startTimes(j,2) = timeWindow2MA{j}(1);
            endTimes(j,2) = timeWindow2MA{j}(end);
            totalJump(j,2) = closestElecInfo2{j}(timeWindow2MA{j}(end)) - closestElecInfo2{j}(timeWindow2MA{j}(1));
            startHop(j,2) = closestElecInfo2{j}(timeWindow2MA{j}(1));
        end
    end
    disp(['Step 12: Feature Extraction for pattern ' num2str(stimElec)]);
    
    % Step 13: Bundle detection without weights
    limit_TW1MA_NoWeight = 2;
    disp(['Step 13: Bundle detection B1 no weights.']);
    endAmp = stimAmpSize-1;
    for j = stimAmpSize-1:-1:2
        if ~isempty(timeWindow1MA{j})
            endAmp = j;
            break;
        end
    end
    inter = timeWindow1MA{endAmp};
    for j = endAmp:-1:2
        inter = intersect(inter, timeWindow1MA{j});
        if (length(inter) <= 1)
            limit_TW1MA_NoWeight = j+1;
            break;
        end
    end
    
    % Band 2
    limit_TW2MA_NoWeight = 2;
    disp(['Step 13: Bundle detection B2 no weights.']);
    inter = timeWindow2MA{endAmp};
    for j = endAmp:-1:2
        inter = intersect(inter, timeWindow2MA{j});
        if (length(inter) <= 1)
            limit_TW2MA_NoWeight = j+1;
            break;
        end
    end
    
    % Step 14: Computation of the max hops for each electrode (used for jumps)
    disp('Step 14: Max Hops Computation');
    MaxHops = zeros(1,2);
    
    if AxonDirection == 1 % compute max hops in the up-down direction
        I = find(unique(yCoords) == yCoords(stimElec));
        MaxHops(1,1) = length(unique(yCoords)) - I;
        MaxHops(1,2) = I;
    else
        % Fix the row, so look at hops in terms of 32 electrodes
        currentRow = unique(xCoords(yCoords == yCoords(stimElec)));
        I = find(currentRow == xCoords(stimElec));
        if angles(partition(stimElec)) < 0
            MaxHops(1,1) = I;
            MaxHops(1,2) = length(currentRow) - 1;
        else
            MaxHops(1,1) = length(currentRow) - 1;
            MaxHops(1,2) = I;
        end
    end
    ...
    SpreadData = cell(1);
    Growth_res = 0;
    disp(['Step 15: Growth test']);
    SpreadTemp = zeros(endAmp,1);
    [bundleMeans] = getBundleVoltagesAStarStartEndFromEndAmpl_Fake(modifiedData);
    
    SpreadTemp(2:endAmp) = squeeze(bundleMeans(1:endAmp-1));
    SpreadData{1} = SpreadTemp;
    data = SpreadData{1};
    ...
    Jump1_res = cell(1);
    Jump1_res{1} = squeeze(totalJump(1:endAmp,1));
    % Also keep track of jumps sequence, so can use in test
    Band1_idx = limit_TW1MA_NoWeight;
    
    Jump2_res = cell(1);
    Jump2_res{1} = squeeze(totalJump(1:endAmp,1));
    % Also keep track of jumps sequence, so can use in test
    Band2_idx = limit_TW2MA_NoWeight;
    ...
    if Dominant_Hops <= 2 % CASE 0 : Too close to border
        Bidir = NaN;
    elseif Dominant_Hops >= Nondominant_Hops
        indexD = JumpsFunction(Dominant_jumps{1}, Dominant_idx, Dominant_Hops);
        indexND = JumpsFunction(Nondominant_jumps{1}, Nondominant_idx, Nondominant_Hops);
        if indexD <= indexND % CASE 1: Use Dominant...
            if indexD <= Growth_res
                Bidir = indexD;
            else
                Bidir = NaN; % Use growth test otherwise
            end
        else % CASE 2: Compare indexND, indexD with growth test
            if max(indexD, indexND) <= Growth_res
                Bidir = max(indexD, indexND);
            else
                Bidir = NaN; % Use growth test otherwise
            end
        end
        ...
        indexD = JumpsFunction(Dominant_jumps{1}, Dominant_idx, Dominant_Hops);
        indexND = JumpsFunction(Nondominant_jumps{1}, Nondominant_idx, Nondominant_Hops);
        if indexD <= indexND % CASE 3: use ND
            if indexND <= Growth_res
                Bidir = indexND;
            else
                Bidir = NaN;
            end
    ...
    % Output the values
    Algorithm_Output(i-2) = Bidir;
    
    alldistances(i-2) = modifiedData.distance_stimElec_to_soma;
    alleisizes(i-2) = modifiedData.mean_axon_signal; 
    if (Algorithm_Output(i-2)-Ground_Output(i-2)) > 1
        disp('Algorithm got a higher estimate on the synthetic data than on the original data for threshold'); 
        disp(dirinfo(i).name); 
    end
%     algorithmResult(i-2) = zeros(size(dirinfo,1)-2,1);
%     algorithmMovie(i-2) = zeros(size(dirinfo,1)-2,1);
%     firstAddedMovie(i-2) = zeros(size(dirinfo,1)-2,1);
%     alldistances(i-2) = modifiedData.;
    
end

%% Plotting to display results
amps = zeros(39,1); 
amps(1) = 0.1; 
for a = 2:length(amps)
    amps(a) = amps(a-1)*1.1; 
...
figure; scatter(Algorithm_Output,Ground_Output); 
[cmap] = cbrewer('seq', 'OrRd',64,'pchip'); 
cmap(1,:) = 1; 
centers{1} = 15:1:39; 
centers{2} = centers{1}; 
[values, centers] = hist3([Algorithm_Output Ground_Output],centers);
figure;  imagesc(centers{:}, (values))
colorbar; colormap(cmap); axis equal; axis xy;
hold on; scatter(Ground_Output,Algorithm_Output,'.k');
hold on; plot(centers{1},centers{2},'--k');
xlim([centers{1}(1) centers{1}(end)]);
ylim([centers{2}(1) centers{2}(end)]);
xlabel('ground');
ylabel('algorithm');
...
[cmap] = cbrewer('seq', 'OrRd',64,'pchip'); 
cmap(1,:) = 1; 
centers{1} = linspace(amps(18),amps(39),40); 
centers{2} = centers{1}; 
[values, centers] = hist3([amps(Algorithm_Output) amps(Ground_Output)],centers);
figure;  imagesc(centers{:}, (values))
colorbar; colormap(cmap); axis equal; axis xy;
hold on; scatter(amps(Ground_Output),amps(Algorithm_Output),'.k');
hold on; plot(centers{1},centers{2},'--k');
xlim([centers{1}(1) centers{1}(end)]);
ylim([centers{2}(1) centers{2}(end)]);
xlabel('ground');
ylabel('algorithm');
%%
binEdges = 0.75:0.1:1.25;
figure; h=histogram(amps(Algorithm_Output)./amps(Ground_Output),'Normalization',...
    'probability'); 
h.BinEdges = binEdges; 
xlabel('threshold ratio alg./ground truth'); 

successes = Algorithm_Output - Ground_Output; 
idx = find(successes==0); 
idx2 = find(successes~=0); 
figure; histogram(alldistances(idx)); hold on; histogram(alldistances(idx2)); 
xlabel('distance to soma'); 
figure; histogram(alleisizes(idx));  hold on; hh = histogram(alleisizes(idx2)); 
hh.BinWidth = 10; 
xlabel('ei strength'); 
legend('detected accurately', 'not'); 