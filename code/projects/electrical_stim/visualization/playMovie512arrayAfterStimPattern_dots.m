...
%                movieIndex - index of the movie to play
...
%                cmap - can be a matlab colormap name (jet, etc) or an n by
%                       3 matrix defining the colormap. 
%                artifactTrials - subtracts the mean of the specified traces
%                            and plots the remaining trials only. Must
%                            specify a single movie for this to work. 
...
cmap = p.Results.cmap; 
artifactTrials = p.Results.artifactTrials; 
...
if ~isempty(artifactTrials)
    if length(mIndices) == 1
        dataTraces=NS_ReadPreprocessedData(pathToAnalysisData, '', 0, patternNo,...
            movieNos(mIndices), 99999);
    
        spikeTrials = setdiff(1:25,artifactTrials); 
        firstArtifact = mean(dataTraces(artifactTrials,:,:),1);
...
for movieIndex = mIndices
    ...
    %keyboard; 
    if ~isempty(artifactTrials)
        dataTraces = dataTraces(spikeTrials,:,:); 
    end
    ...
    if hacking
        test = dataTraces - subtractionMatrix;
        figure; subplot(1,2,1); 
        plot(squeeze(test(:,patternNo,:))'); 
        title('spike trials pre-alignment'); 
        mins = min(test(:,patternNo,:),[],3);
        for ii  = 1:length(mins)
            minIndex(ii) =find(test(ii,patternNo,:) == mins(ii),1); 
        end
        newMinIndex = max(minIndex); 
        for ii  = 1:length(mins)
        if minIndex(ii) < newMinIndex
            test(ii,:,:) = circshift(test(ii,:,:),[0,0,newMinIndex - minIndex(ii)]); 
        end
        end
        subplot(1,2,2); plot(squeeze(test(:,patternNo,:))');
...
set(f,'Color','white');
    for t = 1:40 %size(dataTraces,3)
        ...
        subset = 1:size(positions,1); 
        
        ah = scatter(positions(subset,1),positions(subset,2),circleSize,meanData(subset),'filled'); 
