...
im1 = imread('/Volumes/Analysis/2015-10-06-6/image analysis/bundle_paper_figures/vasc_tranformed.tif'); 
im2 = imread('/Volumes/Analysis/2015-10-06-6/image analysis/bundle_paper_figures/tubulin_tranformed.tif'); 
im3 = imread('/Volumes/Analysis/2015-10-06-6/image analysis/bundle_paper_figures/dapi_tranformed.tif'); 
arr = imread('/Volumes/Data/2015-10-06-6/Imaging/vasculature_alignment_with_array/2015-10-06-6_stitched_sm.jpg');
array = arr(650:5340,1045:9445,3);
...
hsize = 80;% 120; % 241=60 um separation of electrodes.
sigma = 0.45;
...
load('/Volumes/Analysis/2015-10-06-6/image analysis/bundle_paper_figures/electrode_coords.mat');
...
maskOfBundles(filtered >100) = 255; 
figure; imshow(maskOfBundles); hold on; 
for row = 4:12
    elecs = array_matrix_id510(row,:);
    elecs = elecs(5:22);
    scatter(newXYCoords(1,elecs),newXYCoords(2,elecs),100,'r'); 
end

cropped_image_row = 1110:3485;
cropped_image_col = 1100:5600;
figure; imshow(maskOfBundles(cropped_image_row,cropped_image_col)); hold on; 

for row = 4:12
    elecs = array_matrix_id510(row,:);
    elecs = elecs(5:22);
    scatter(newXYCoords(1,elecs)-cropped_image_col(1),newXYCoords(2,elecs)-cropped_image_row(1),100*bundleThresholds_2015_10_06_6(elecs),[0 1 .2 ],'filled'); 
end

figure; imshow(imadjust(filtered(cropped_image_row,cropped_image_col))); hold on; 
for row = 4:12
    elecs = array_matrix_id510(row,:);
    elecs = elecs(5:22);
    scatter(newXYCoords(1,elecs)-cropped_image_col(1),newXYCoords(2,elecs)-cropped_image_row(1),100*bundleThresholds_2015_10_06_6(elecs),[0 1 .2 ],'filled'); 
...
% imwrite(imadjust(filtered(cropped_image_row,cropped_image_col)),'/Users/grosberg/Dropbox/bundle_image_correlation/filtered_image.tif','tiff')
% imwrite(maskOfBundles(cropped_image_row,cropped_image_col),'/Users/grosberg/Dropbox/bundle_image_correlation/bundle_mask.tif','tiff')

%%

radius = 120; %pixels, 121 is equal to 30 microns.

radii = [20 30 40 60 80 100 120 140 160 180 200 240]
allCorrCoeffs = zeros(length(radii),2); 
[row_idx,col_idx]= meshgrid(1:size(maskOfBundles,2),1:size(maskOfBundles,1)); 
for rr = 4%1:length(radii)
radius = radii(rr); 
...
%         fluo_vals(ii) = mean(filtered(C)); 
        fluo_vals(ii) = mean(maskOfBundles(C)); 
...
figure; scatter(fluo_vals,bundle_ts) 
xlabel(sprintf('Mean intensity in a %0.0f um radius',radius/121*30)); 
ylabel('axon bundle activation threshold (\muV)'); 
[r,p] = corrcoef(fluo_vals,bundle_ts);
title(sprintf('Correlation is %0.4f with p value of %0.3f',r(2),p(2))); 
allCorrCoeffs(rr,:) = [r(2) p(2)]; 
end
...
figure; scatter(radii(2:end)/121*30, abs(allCorrCoeffs(2:end,1)),100,'k','filled'); 
