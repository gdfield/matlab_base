...
useStimElec=params.global.useStimElectrodeBeforeBundle+params.global.useStimElectrodeAfterBundle;
...
for i=1:maxCond
    ...
    while(flag==1&&cont<=maxIter)
        ...
        if((cont>1)&&(useStimElectrodeBeforeBundle+useStimElectrodeAfterBundle>=1))
            els2=els;
    ...
    for e=1:size(elExtra(i,:),2)
        ...
        indpat=union(indpat,find(elsExtras(e)==els));
    end
    
    for j=1:length(indpat)
        indels=setdiff(indels,indpat(j):length(els):Tmax*length(els));
    end
    end  
