function [TracesAll Art var0 listAmps listCurrents onset onsetC pval Res stimElecs firstArt]=loadTracesArt(pathToAnalysisData,patternNo,Tmax,nTrial,varargin)
...
disp('Only has the default number of electrodes, 512'); 
if(nargin==5)
    params=varargin{1};
...
TracesAll=NaN*zeros(length(movieNos),nTrial,numElecs,Tmax);
...
for m=1:length(movieNos);
    ...
    a=TracesAll(m,:,setdiff([1:numElecs],patternNo),:);
    if(m<=5)
        varm(m)=nanvar(a(:));            
    end
...
if(findBundle)   
    ...
    pats=arrayObj.getNeighbors(stimElecs(1),nNeighborsExclude);
    nConds=size(Art,1);
    pval = nan(nConds,1);
    for k=1:nConds
        [h, p]=kstest((log(Res(k,setdiff([1:numElecs],pats)))-nanmean(log(Res(k,setdiff([1:numElecs],pats)))))./nanstd((log(Res(k,setdiff([1:numElecs],pats))))));
        pval(k)=log(p);
    end
