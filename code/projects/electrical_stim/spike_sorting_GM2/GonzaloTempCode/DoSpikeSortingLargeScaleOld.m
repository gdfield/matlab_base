function [Output]=DoSpikeSortingLargeScale(pathToPreparation,patternNo,neuronIds,params,templates,arrayObj,varargin)
...
if(params.global.sortData)
    [TracesAll, Art, var0, listAmps, listCurrents, stimElecs, onset, onsetC, pval, Res, sampledTrials, Trace0, polarity firstArt channelsConnected]=loadTracesArtSort(pathToAnalysisData,patternNo,Tmax,nTrial,params.global.subSampleRate,arrayObj,'params',params);
    
else
    [TracesAll, Art, var0, listAmps, listCurrents, onset, onsetC, pval, Res, stimElecs firstArt]=loadTracesArt(pathToAnalysisData,patternNo,Tmax,nTrial,params);
...
%     f1=@(Art,y)logDetKron(Art(1:onsetC-1,ind,:),[x(1:9) y(1) log(var0old)],DifAux,[10],[1 1 1],DiagsAux,[3 3 3]);
%     g1=@(y)f1(Art,y);
...
x(10)=x(10)-params.arrayInfo.xwhiten(3);
params.arrayInfo.x=x;
...
if(useKernels)
    ...
    [spikes Log params]=SpikeSortingNoKer(params,TracesAll);
end
