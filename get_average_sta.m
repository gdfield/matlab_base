function avg_sta = get_average_sta(datarun, cell_spec, varargin)
% get_average_sta     Average a collection of stas together.
%
%   before averaging, each sta is linearly interpolated in space using the provided
%   scale factor, and re-centered.  
% 
%
% usage:  avg_sta = get_average_sta(datarun, cell_spec, varargin)
%
% arguments:  datarun - datarun struct
%           cell_spec - which cells (see get_cell_indices for options)
%            varargin - struct or list of optional parameters (see below)
%
% outputs:     avg_sta - average sta
%
%
...
p.addParamValue('normalize', true); %%%% add var option
p.addParamValue('center', []);
p.addParamValue('which_rfs', 'rfs');
p.addParamValue('which_center',[]);
...
% identify location to which all spatial stas will be averaged
...
% initialize avg_sta
t = datarun.stas.stas{1};
avg_sta = zeros(size(t,1)*params.scale,size(t,2)*params.scale,1,size(t,4));
...
for cc = 1:length(cell_indices)
    ...
    % get RF center
    if isfield(params,'which_center')
        rf_ctr = rf_center(datarun,cell_id,params.which_center);
    else
        rf_ctr = rf_center(datarun,cell_id);
    end
    
    % skip if empty
    if isempty(sta) || isempty(rf_ctr);continue;end
    
    % normalize sta, if desired
    if params.normalize
        sta = sta/max(sta(:)); %%%%% add var option
    end
    ...
    for fr = 1:size(sta,4)
        ...
        avg_sta(:,:,1,fr) = avg_sta(:,:,1,fr) + new_rf;
    end
