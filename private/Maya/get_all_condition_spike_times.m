function [results, datarun1] = get_all_condition_spike_times(datarun1, cell_ids, trigger_interval, varargin)
...
% guarantee datarun.stimulus.repetitions exists, since downstream
% functions (testraster, browse_rasters, saverasters) rely on it, and
% not every version of load_stim sets it
trial_counts = histcounts(datarun1.stimulus.trial_list, 0.5:1:(length(datarun1.stimulus.combinations) + 0.5));
computed_reps = mode(trial_counts);

if ~isfield(datarun1.stimulus, 'repetitions') || isempty(datarun1.stimulus.repetitions)
    datarun1.stimulus.repetitions = computed_reps;
elseif datarun1.stimulus.repetitions ~= computed_reps
    warning('get_all_condition_spike_times:repMismatch', ...
        'datarun.stimulus.repetitions (%d) does not match the actual trial counts (%d) - using the actual count.', ...
        datarun1.stimulus.repetitions, computed_reps);
    datarun1.stimulus.repetitions = computed_reps;
end
