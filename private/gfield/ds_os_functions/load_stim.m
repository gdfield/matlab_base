...
clear s
            ...
            % if class was CG, convert spatial phase from pixel offset to
%             % degrees
%             % use change entry??
%             if text(10)=='C' && text(11)=='G'
%                 for t=1:length(s.trials)
%                     pixel_offsets(t)=360*s.trials(t).SPATIAL_PHASE/s.trials(t).SPATIAL_PERIOD;
%                 end
%                 pixel_offsets=round(pixel_offsets);
% %                 pixel_offsets=unique(pixel_offsets); %check
%                 for t=1:length(s.trials)
%                     s.trials(t).SPATIAL_PHASE=pixel_offsets(t);
%                 end
%             end
%   
