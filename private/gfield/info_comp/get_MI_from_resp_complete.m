function [I,csr]=get_MI_from_resp_complete(spikes,bins,word_size,starts,samples)
...
for s=1:length(samples)
    ...
    %split up spikes
    for r=1:runs
        subsampled_spikes{r}=spikes(1+(r-1)*trials_per_run:trials_per_run*r,:);
    end
    for r=1:runs
        subsampled_MI(r)=get_MI_from_resp(subsampled_spikes{r},bins,word_size,starts(1+(r-1)*trials_per_run:trials_per_run*r));
    end
