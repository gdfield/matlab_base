...
for spat_p = 1:num_sps_c %WHAT ARE THE LEVELS WITHIN THE FOR LOOP? 
    for temp_p = 1:num_tps_c
                ...
                temp_tp_c = temp_periods_c(temp_p);
                fig_title = ['cell id: ', num2str(datarun_dg_c.cell_ids(control_dg_index)), ' SP: ', num2str(temp_sp), '; TP: ', num2str(temp_tp_c)];
...
for spat_p = 1:num_sps_b
    for temp_p = 1:num_tps_b
                ...
                temp_tp_c = temp_periods_b(temp_p);
                fig_title = ['cell id: ', num2str(datarun_dg_b.cell_ids(block_dg_index)), ' SP: ', num2str(temp_sp), '; TP: ', num2str(temp_tp_c)];
...
%for hOS cell 9 (2021-09-09-0 NDF0 control v block), used SP = 80 and TP = 60
%for vOS cell 14 (2021-09-09-0 NDF0 control v block), used SP = 80 and TP = 60
...
temp_sp_c = find(spatial_periods_c == SP);
temp_tp_c = find(temp_periods_c == TP);

temp_spike_times = datarun_dg_c.spikes{control_dg_index};
[temp_tuning, temp_spike_nums, temp_rates] = get_direction_tuning(temp_spike_times, datarun_dg_c.stimulus,...
                                        'SP', spatial_periods_c(temp_sp_c), 'TP', temp_periods_c(temp_tp_c));
plot_direction_tuning(temp_tuning, temp_spike_nums, datarun_dg_c.stimulus,...
            'fig_num', fig_counter, 'print_for_fig', false)  
exportgraphics(gcf,'control.pdf','ContentType','vector')
%% print for block condition 
temp_sp_b = find(spatial_periods_b == SP);
temp_tp_b = find(temp_periods_b == TP);        

temp_spike_times = datarun_dg_b.spikes{block_dg_index};
[temp_tuning, temp_spike_nums, temp_rates] = get_direction_tuning(temp_spike_times, datarun_dg_b.stimulus,...
                                        'SP', spatial_periods_b(temp_sp_b), 'TP', temp_periods_b(temp_tp_b));
plot_direction_tuning(temp_tuning, temp_spike_nums, datarun_dg_b.stimulus,...
            'fig_num', fig_counter, 'print_for_fig', false)  
exportgraphics(gcf,'drug.pdf','ContentType','vector')        
