...
OSI_thresh = 0.45; % threshold of OS cells
...
for rgc = 3:num_rgcs
    ...
    if isempty(sig_OSIs)
        ...
        if ~isempty(insig_DSI)
            ...
            if found_flag == 1
                [mapped_list, failed_list] = map_ei(datarun, datarun_wn, 'master_cell_type', os_cell_list(rgc), 'corr_threshold', corr_treshold);
                temp_cell_index = get_cell_indices(datarun, os_cell_list(rgc));
            else
            ...
            if found_flag == 0
                ...
                while input_check == 0
                    ...
                    if strcmp(u_reply, 'y')
                        ...
                        OSI_matrix = [OSI_matrix, OSI];
                        input_check = 1;
                        temp_os_type = input('h or v:', 's')
                        os_type = [os_type, temp_os_type];
...
print(10, '~/Desktop/h_rf.pdf', '-dpdf')

figure(20); clf;
spatial_frequencies = [5 10 20 40 80 160 320];
plot(spatial_frequencies, OSI_matrix(:,15), 'k')
xlabel('spatial frequency');
ylabel('OSI')
axis([0 330 0 1])
axis square
print(20, '~/Desktop/htuning.pdf', '-dpdf')
