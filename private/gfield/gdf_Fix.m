...
addpath(genpath('/Volumes/dusom_fieldlab/All_Staff/lab/Development/matlab/private/jcafaro/'))
addpath(genpath('/Volumes/dusom_fieldlab/All_Staff/lab/Manuscripts/2019/Natural_global_motion_ms/'))
...
saveFigPath = ['~/Desktop/'] ;
saveDsIdsPath = ['/Volumes/dusom_fieldlab/All_Staff/lab/Manuscripts/2019/Natural_global_motion_ms/mat_files/DsIdsDb',num2str(DB),'ConcatPathNum',num2str(ConcatPathNum),'DsPathNum',num2str(DsPathNum)] ;
...
% grating vector sum
for cells=1:length(dataRun.spikes) ; % for each cell
    VectTemp = NaN(NumDirDg,2) ; % prep mat
    for tmp = 1:NumTempPeriods ; % for each temp period
...
% parameters defining population to decoding from
LocalPopFlag = false ; % use local population
UseQuadFlag = false ; % use single quad only
ExcludeOnDSFlag = true ; 
UseDsCellsOnlyFlag = true ; 
UseNonDsCellsOnlyFlag = false ;
LocalPopRad = 100 ; % (um) 125
SeedSpot = [225,80]; %[120,120]; %[225,80] ; % [0,0] ; % center of circle

% parameters defining decoding methods
OqeFlag = false ; % true = use OQE, false use OLE
RegularizeOqeFlag = false ; % true = regularize OQE
RegLambda = 15 ; % value used for regularization
...
ShuffTrialTestFlag = true ; % shuffle trials
...
% decode
if UseQuadFlag ;
    looplength = length(RealQuad_i) ; 
else
    looplength = 1 ;
end

for Quad_loop = [1:looplength] ; % for each real quad
    RealQuadToUse = Quad_loop ; % 
    ...
    for im = 1:NumImages ; % for each Ni image 
        ...
        TrainingIm_Ni = setdiff([1:NumImages],im) ; % 
        TrainingTrials_Ni = [1:NumCompleteTrials] ; % setdiff([1:size(psthNi{1}{1},1)],[6:6:size(psthNi{1}{1},1)]); % [1,5]
        TrainingPnts_Ni = SteadyStatePntsNi; % SteadyStatePntsNi(randperm(length(SteadyStatePntsNi),2000)) ; % randperm(size(psthNi{1}{1}{1},2)-StimOnOffsetPnts,1000)+StimOnOffsetPnts; % randperm(size(psthNi{1}{1},2),2000) ; %[(StimOnOffsetPnts+1):length(psthNi{1}{1}(1,:))-StimOffPnts] ;
...
%         end
        ...
        % prep mats
        DirVect = NaN(NumAllPnts,1) ; % prep mat 
        RspMat = NaN(NumAllPnts,NumOleCells) ; % prep mat
        ...
        TestingPnts_Ni = SteadyStatePntsNi ; %setdiff(SteadyStatePntsNi,TrainingPnts_Ni)
        ...
        for st = 1:NumDirNi ; % for each direction 
            ...
            for t=1:NumCompleteTrials ; % for each trial
                ...
                RspMatTest = NaN(length(psthTimeNi),NumOleCells) ; % prep mat
            ...
            HeldOutError(st,im,:,:) = DirEstimate_Error{st}{im}(TestingTrials_Ni,TestingPnts_Ni) ; % error for non-trained data
            HeldOutMag(st,im,:,:) = DirEstimate_mag{st}{im}(TestingTrials_Ni,TestingPnts_Ni) ; % error for non-trained data
        end
    end
    ...
    for st = 1:NumDirNi ; % for each stimulus 
        for im = 1:NumImages ; % for each image to test
            Temp = [Temp,DirEstimate_Error{st}{im}(TestingTrials_Ni,TestingPnts_Ni)] ; % error where tested
        end
    end
    ...
    for st = 1:NumDirNi ; % for each stimulus 
        for im = 1:NumImages ; % for each image to test
            Temp = [Temp,DirEstimate_mag{st}{im}(TestingTrials_Ni,TestingPnts_Ni)] ; % mag where tested
        end
    end
    MagAll = Temp(:) ;
    ...
    ErrorMed_QuadLoop(Quad_loop) = ErrorMed ;
    ErrorBestMedian_QuadLoop(Quad_loop) = ErrorBestMedian ;
