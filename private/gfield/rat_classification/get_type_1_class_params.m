function class_params = get_type_1_class_params(datarun_wn, datarun_dg, cell_ids_wn, cell_ids_dg)

epoch_duration = 8;
num_epochs = length(datarun_dg.epoch_triggers_256);
% check that cell_ids lengths are the same
if cell_ids_wn ~= cell_ids_dg
    error('cell_ids for the white noise and grating conditions are not equal in length')
end
